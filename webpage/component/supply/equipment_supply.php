...
<body>
  ...
  <div id="mainWrapper">
    ...
    <!-- 게시판 목록  -->
      <ul id="ulTable">
       ...
       $info_sql = "select equip_name,amount,total,cinema_id from equipment natural join cinema where cinema_id='$cinema_name'";
       $info_stt=$db->prepare($info_sql);
       $info_stt->execute();
       foreach($info_stt as $info){
         $temp = $info['total'] *500;
         if($info['equip_name']=='유니폼')
          $temp = $info['total']*15000;
        ?>
        <li>
          <ul>
            <li><?= $info['equip_name']; ?></li>
            <li><?= $info['amount']; ?></li>
            <li><?= $info['total']; ?></li>
            <li><?= $temp; ?></li>
          </ul>
  ...
  <button type="button" onclick="location.href='equipmentbuy.php'">주문하러가기</button>
</div>
</body>
